"""Exotel telephony routes (webhooks, status callbacks, answer URLs).
...
async def _handle_exotel_status_callback(
    workflow_run_id: int,
    request: Request,
):
    ...
    logger.info(
        f"[run {workflow_run_id}] Received Exotel callback: {json.dumps(callback_data)}"
    )
    ...
    if not is_valid:
        logger.warning(f"[run {workflow_run_id}] Invalid Exotel webhook signature")
...
async def handle_exotel_xml_webhook(
    ...
    Returns Exotel XML response with Stream element.
    """
    set_current_run_id(workflow_run_id)
    workflow_run = await db_client.get_workflow_run_by_id(workflow_run_id)
    ...
    if not is_valid:
        logger.warning(
            f"[run {workflow_run_id}] Invalid Exotel signature on answer webhook"
        )
...
async def handle_exotel_hangup_callback(
    ...
    """Handle Exotel hangup callbacks."""
    return await _handle_exotel_status_callback(workflow_run_id, request)


@router.post("/exotel/ring-callback/{workflow_run_id}")
async def handle_exotel_ring_callback(
    workflow_run_id: int,
    request: Request,
):
    """Handle Exotel ring callbacks."""
    return await _handle_exotel_status_callback(workflow_run_id, request)
