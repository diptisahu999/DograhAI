...
class ExotelProvider(TelephonyProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = f"https://api.exotel.com/v1/Account/{self.auth_id}"
    ...
    async def initiate_call(
        ...
        **kwargs: Any,
        ...
        if workflow_run_id:
            ...
            data.update(
                {
                    "hangup_url": f"{backend_endpoint}/api/v1/telephony/exotel/hangup-callback/{workflow_run_id}",
                    "hangup_method": "POST",
                    "ring_url": f"{backend_endpoint}/api/v1/telephony/exotel/ring-callback/{workflow_run_id}",
                    "ring_method": "POST",
                }
            )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(endpoint, json=data, auth=auth) as response:
                ...
                if not call_id:
                    raise HTTPException(
                        status_code=500,
                        detail=f"Exotel response missing call identifier: {response_data}",
                    )
    ...
    def _stringify_signature_value(value: Any) -> Any:
        ...
        if isinstance(value, list):
            return [ExotelProvider._stringify_signature_value(item) for item in value]
        return value

    @staticmethod
    def _query_map(query: str) -> Dict[str, Any]:
        return {
            ExotelProvider._stringify_signature_value(
                key
            ): ExotelProvider._stringify_signature_value(value)
    ...
    def _sorted_query_string(params: Dict[str, Any]) -> str:
        ...
        for key in sorted(params.keys()):
            ...
            if isinstance(value, list):
                ...
                parts.append(f"{key}={ExotelProvider._stringify_signature_value(value)}")
        return "&".join(parts)
    ...
    def _sorted_params_string(params: Dict[str, Any]) -> str:
        ...
        for key in sorted(params.keys()):
            ...
            if isinstance(value, list):
                ...
                parts.append(f"{key}{ExotelProvider._sorted_params_string(value)}")
            else:
                parts.append(f"{key}{ExotelProvider._stringify_signature_value(value)}")
        return "".join(parts)
    ...
    def _construct_get_url(
        ...
        combined_params.update(ExotelProvider._query_map(parsed_uri.query))
        query_params = ExotelProvider._sorted_query_string(combined_params)
    ...
    def _construct_post_url(uri: str, params: Dict[str, Any]) -> str:
        base_url = ExotelProvider._construct_get_url(
            uri,
            {},
            empty_post_params=(len(params) == 0),
        )
        return f"{base_url}{ExotelProvider._sorted_params_string(params)}"
...
<Response>
    ...
    async def handle_websocket(
        ...
        if not call_id:
            logger.error(f"Missing call ID for Exotel workflow run {workflow_run_id}")
    ...
    def can_handle_webhook(
        ...
        has_exotel_signature = (
            "x-exotel-signature-v3" in headers or "x-exotel-signature-ma-v3" in headers
        )
        return has_exotel_signature and "CallUUID" in webhook_data
    ...
    def validate_account_id(config_data: dict, webhook_account_id: str) -> bool:
        ...
        # AuthID is not always present in Exotel webhooks (undocumented field).
        # Fall back to checking that the org has a Exotel config at all.
        logger.warning(
            "Exotel webhook missing AuthID/ParentAuthID - "
    ...
    async def verify_inbound_signature(
        ...
        signature = headers.get("x-exotel-signature-v3") or headers.get(
            "x-exotel-signature-ma-v3", ""
        )
        nonce = headers.get("x-exotel-signature-v3-nonce", "")
        if not signature:
            # Exotel always signs its webhooks; missing header means the
            # request didn't come from Exotel (or was tampered with).
            logger.warning("Inbound Exotel webhook missing X-Exotel-Signature-V3")
            return False
    ...
    async def configure_inbound(
        ...
        """Update the answer_url on the configured Exotel Application.

        Exotel numbers don't carry an answer_url directly — the URL lives on a
        Exotel Application, and a number is linked to one app via ``app_id``.
        ...
        ``self.application_id`` (in the Exotel console, or via the Account
        Phone Number API) is the operator's responsibility — we only update
        the application's webhook here.

        Clearing (``webhook_url=None``) is a no-op on Exotel's side: the URL
        ...
        if webhook_url is None:
            logger.info(
                f"Exotel configure_inbound clear for {address}: skipping "
        ...
        if not self.validate_config():
            return ProviderSyncResult(
                ok=False, message="Exotel provider not properly configured"
            )
        ...
        if not self.application_id:
            return ProviderSyncResult(
                ok=False,
                message=(
                    "Exotel application_id is not configured. Set it in the "
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(app_endpoint, json=data, auth=auth) as response:
                    if response.status not in (200, 202):
                        ...
                        logger.error(
                            f"Exotel application update failed for "
                            f"{self.application_id}: {response.status} {body}"
                        )
                        return ProviderSyncResult(
                            ok=False,
                            message=f"Exotel API {response.status}: {body}",
                        )
        except Exception as e:
            logger.error(
                f"Exception updating Exotel application {self.application_id}: {e}"
            )
            return ProviderSyncResult(ok=False, message=f"Exotel update failed: {e}")

        logger.info(
            f"Exotel answer_url set on application {self.application_id} "
